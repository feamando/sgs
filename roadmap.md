...
| 1-planck-1-3 | Planck 1.3 | model | open | 2026-04-27 | Wikipedia base retrain (~4B tokens, plain AdamW) + static Wikipedia blob index (1.3.1a) + live RSS blob index (1.3.1b, deferred post-Hertz). Plan updated 2026-05-04: corpus is Wikipedia (encyclopedic priors + same corpus doubles as static blob index); wall-clock ~3-7 days on 4090 (Planck 1.1 was ~3 hours on ~1-2B TinyStories tokens, so a 4× corpus scale-up is still under a week). Runs in background during Raum 0.1/1.1 foreground. Details at docs/plans/planck_13_plan.md |
...
| 6-raum-1-1 | Raum 1.1 | model | open | 2026-04-27 | Rewritten 2026-05-01 for the 1-model architecture: frozen Planck encoder + Raum bridge at d_model=256/6L/8H (~5-10M params) + ShapeNet Core v2 blob library (30 classes → 60) + relation graph head + DSL v1 as pure output. No separate Planner LM. 2026-05-04: foreground priority; stages 1.1.A/B start against Planck 1.1 while Planck 1.3 trains in the background, bridge re-trains against 1.3 (hours) once 1.3 is ready. Details at docs/plans/raum_11_plan.md |

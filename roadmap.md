...
| 10-raum-0-7 | Raum 0.7 | product | open | 2026-06-04 | Cheap appearance wins that ship NOW on the existing castle, no new model (independent of 1.7). (1) per-Gaussian spherical harmonics (sh_degree already in GaussianParams) for view-dependent shading + stone texture/noise so flat RGB reads as weathered stone (material axis); (2) adopt a real gsplat renderer (gsplat.js / @mkkellogg/gaussian-splats-3d) fed by the existing .ply/.splat export, replacing the hand-rolled Three.js ellipsoids -> proper anisotropic splatting. Makes whatever geometry we have look its best while 1.7 does the hard learned-geometry work. Plan in SETUP_202606_04.md §0.7 |

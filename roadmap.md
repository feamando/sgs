...
| 6-raum-1-6 | Raum 1.6 | model | open | 2026-06-03 | Compositional accuracy. Retrain the decomposer (Planck 1.3 frozen, decomposer-only -- NOT a base retrain) on a large COMPOSITIONAL grammar: every element as a standalone labeled scene (wall, gate, door, window, arrow-slit, arch, tower, keep, gatehouse, hill, cliff, tree, woods) plus castles as arrangements of them. Prompt-conditioned structure (honor counts + presence/absence: "a castle with two towers", "a wall with a gate", "a fort without trees") so the model stops collapsing onto one template. Grammar-validated constrained decoding (reject/repair unparseable trees, snap part positions to canonical layout). Diversity over volume: ~100-200K diverse records, not GB of near-duplicate castles. No LLM required (optional local LLM for phrasing only). Details at SETUP_20260603.md §3-4 |
| 10-raum-0-6 | Raum 0.6 | product | open | 2026-06-03 | Realism. (1) True anisotropic Gaussian splats: grammar emits oriented ellipsoid stones (flat slab-stones for walls), viewer renders oriented ellipsoids not flat round discs (GaussianParams already carries 3-axis scale + quaternion; consider adopting a real gsplat.js renderer). (2) Real-scan part routing: map each part-leaf to its matched Sketchfab category scan (109 scans / 17 cats already labeled by dir) via the existing Chamfer template path, so a tower is a real scanned tower. (3) Atomic openings: windows/doors/arrow-slits as recessed dark boxes. (4) Cheap shading + AO in the viewer. Multiview refinement optional. Physical Gaussians (e_p) explicitly OUT of scope -> stays Raum 2.0. Details at SETUP_20260603.md §5 |

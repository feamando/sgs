...
| 1-planck-1-2-1 | Planck 1.2.1 | model | in progress | 2026-04-28 | Two-track accel remediation ran 2026-04-28; 4 of 8 runs crashed (muon/all_plus scheduler TypeError, sk OOM step 9.9k, all collapsed then OOM step 29.9k). Two honest regressions (tl, shk both +0.16 nats). Gate unreachable. Three root-cause bugs logged in Planck 1.2.2 |
| 1-planck-1-2-2 | Planck 1.2.2 | model | open | 2026-04-29 | Three-bug fix pass on 1.2.1: (1) `MuonWithAuxAdam` subclass `Optimizer` for scheduler compat, (2) `_causal_render_sparse` flat `index_select` (kills `[B,L,L,d_f]` backward intermediate that caused sk OOM), (3) tl reweight additive floor + batch-mean renorm (multiplicative floor was pinning T at 1.0, collapsing the compound). Minimum-viable 6-row re-matrix: baseline adopted + `sk_fix`, `muon_fix`, `tl_fix`, `all_fix`, `all_plus_fix`. Either-path gate unchanged from 1.2.1. ~12-15h GPU on RTX 4090. Unblocks Hertz 1.2 on pass |
...
| 2-hertz-1-2 | Hertz 1.2 | model | open | 2026-04-20 | Large-LM run with an accel recipe. Blocked on Planck 1.2.2 either-path compound gate (any of `all_fix`, `muon_fix`, `all_plus_fix` clearing the 1.2.1 thresholds). Winning recipe flows into SETUP §6.5 templates A/B/C |

...
| 1-planck-1-3 | Planck 1.3 | model | open | 2026-04-27 | Wikipedia base retrain (~4B tokens, plain AdamW) + static Wikipedia blob index (1.3.1a) + live RSS blob index (1.3.1b). Plan updated 2026-05-01: corpus switched from BabyLM to Wikipedia so the base gets encyclopedic priors AND the same corpus doubles as the static blob index; training wall-clock goes up (~2-3 weeks) without the shelved 1.2 accel recipe. Details at docs/plans/planck_13_plan.md |
...
| 2-hertz-1-2 | Hertz 1.2 | model | open | 2026-04-20 | Large-LM run on plain AdamW (no accel recipe); unblocked 2026-05-01 after Planck 1.2.1/1.2.2 closed FAIL. Runs after Klang 1.3, Satz 0.1, and Raum 0.1/1.1 so the product demos ship before GPU-weeks commit. ~10 days for 10B tokens on RTX 4090 at baseline throughput. Details in SETUP_202605.md §6 |
...
| 6-raum-1-1 | Raum 1.1 | model | open | 2026-04-27 | Rewritten 2026-05-01 for the 1-model architecture: frozen Planck 1.1 encoder + Raum bridge at d_model=256/6L/8H (~5-10M params) + ShapeNet Core v2 blob library (30 classes → 60) + relation graph head + DSL v1 as pure output. No separate Planner LM. Details at docs/plans/raum_11_plan.md |
...
| 10-raum-0-1 | Raum 0.1 | product | open | 2026-04-27 | Rewritten 2026-05-01: 1-model architecture (frozen Planck 1.1 encoder + Raum bridge, no separately-trained Planner LM), 30→60-class ShapeNet blob library, editable DSL v1 for the nano-banana edit loop. Run order: ships AFTER Klang 1.3 and AFTER Satz 0.1 (Planck-1.1-encoder validation), BEFORE Hertz 1.2. Details at docs/plans/d1c_raum_01_plan.md |
| 11-satz-0-1 | Satz 0.1 | product | open | 2026-05-01 | Local web demo of Planck 1.1 LM + blob retrieval. Prompt textbox, streamed generation, right-panel retrieved-blob list with transmittance weight bars, k-slider. No new training. Two roles: product demo on the Satz swimlane AND validation that Planck 1.1 is the right frozen encoder for Raum 1.1 before committing architecturally. ~2-3 sessions. Runs AFTER Klang 1.3, BEFORE Raum 0.1/1.1. Details at docs/plans/satz_01_plan.md |

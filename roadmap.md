...
| 10-raum-0-5 | Raum 0.5 | product | open | 2026-06-01 | Honest castle on a hill. Deterministic scene grammar (parameterized atomic parts: stone/crenellation/tower/gate/keep/hill/tree, with corner-tower + centered-gate spatial rules) renders a recognizable low-fidelity castle with NO model in the loop, via a new --scene-file path in the demo. Proves the fill->densify->refine->render stack can hit the target and doubles as the 1.5 training-data generator. Follows Raum 1.4 post-mortem (shapes still blocks, bottleneck = decomposer). Details at SETUP_202606_2.md §3 |
| 6-raum-1-5 | Raum 1.5 | model | open | 2026-06-01 | Retrain decomposer on the 0.5 grammar's deep atomic trees, narrowed to castle/hill/village/tower domain. Fine-tune frozen Planck 1.3 + adapter (60 epochs, max-len 768), paraphrased prompts, optional tree-level compositional loss (corner towers, centered gate). Goal: learned model emits the same deep structure the 0.5 grammar proves looks right. Blocked on 0.5. Details at SETUP_202606_2.md §4 |

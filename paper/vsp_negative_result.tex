...
\usepackage{lastpage}
% \jmlrheading is filled in by the JMLR production editor on acceptance
% (volume, pages, submitted/published dates, paper id). Leave commented for the
% review copy so no placeholder volume/date header is printed.
...
A remaining objection is that the deficit reflects a broken initialization pipeline rather than uninformative grounding: perhaps any structured, non-random initialization would hurt equally. We control for this by initializing from a \emph{scrambled} bundle, the same grounded vectors permuted across tokens so that each token receives a real bundle belonging to a different word, preserving the norm and covariance of the initialization while destroying its correspondence to the token. If the true bundle carries transferable sense information, it should beat the scrambled one. At $2$k steps it does, and significantly: grounded $0.695$ versus scrambled $0.590$, a difference of $+10.5$ points ($13$ gains against $2$ losses among $15$ discordant pairs, exact McNemar $p = 0.007$). The grounding signal is therefore genuine and the pipeline is sound; a permuted bundle is a materially worse initialization than the true one. At $40$k steps, however, the advantage decays to $+2.9$ points and is no longer significant ($p = 0.51$): grounded $0.790$ versus scrambled $0.762$, with the random-initialization baseline ($0.829$) now above both. The advantage of the true bundle over the scrambled one shrinks from $+10.5$ points (significant) at low compute to a non-significant $+2.9$ at full compute, and at full compute a plain random start outperforms either structured initialization. This is direct evidence for washout, and stronger than the output correlation: an initialization that measurably matters early ceases to matter once the model has enough gradient signal to learn the distinction from text alone.
...
repository at \url{https://github.com/feamando/sgs}.}

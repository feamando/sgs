...
theorem convex_combination_bounded {n : ℕ} (w : Fin n → ℝ) (f : Fin n → ℝ)
  ...
  exact ⟨ Finset.sum_nonneg fun i _ => mul_nonneg ( hw_nonneg i ) ( hf_bounds i |>.1 ), hw_sum ▸ Finset.sum_le_sum fun i _ => mul_le_of_le_one_right ( hw_nonneg i ) ( hf_bounds i |>.2 ) ⟩

-- Aristotle: sum of nonneg products is nonneg; upper bound by replacing f with 1
...
theorem material_similarity_bounded_and_self_maximal
    ...
    (_hf_bounded : ∀ i, 0 ≤ f_self i ∧ f_self i ≤ 1) :
    ∑ i, w i * f_self i = 1 := by
  aesop
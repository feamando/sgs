...
\begin{equation}
w_i^{\text{alpha}} = a_i \cdot T_i = \frac{w_i}{R_i} \cdot R_i = w_i \qedhere
\end{equation}
\end{proof}
...
This is relevant to the Semantic Gaussian Splatting (SGS) proposal~\cite{gorshkov2025}, which replaces transformer attention with the alpha-compositing rendering equation from 3D Gaussian Splatting. The theorem proves this replacement does not lose expressiveness.
...
\bibitem{gorshkov2025}
N.~Gorshkov.
\newblock Semantic {G}aussian splatting.
\newblock Radiance Labs, 2025.
